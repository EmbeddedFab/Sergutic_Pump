...
const UART_reg_str uart_reg = {&UDR0, &UCSR0A ,&UCSR0B , &UCSR0C, &UBRR0H, &UBRR0L};
...
void EF_void_UART_Init(UART_cfg_str *uart_cfg)
{
	...
	*uart_reg.UBRRL_Reg = (uint8_t)uart_UBBR;
	...
	if (uart_cfg->RXInterruptEnable) *uart_reg.UCSRB_Reg |= (1 << RXCIE0);
	if (uart_cfg->TXInterruptEnable) *uart_reg.UCSRB_Reg |= (1 << TXCIE0);
	if (uart_cfg->ReceiverEnable   ) *uart_reg.UCSRB_Reg |= (1 << RXEN0);
	if (uart_cfg->TransmitterEnable) *uart_reg.UCSRB_Reg |= (1 << TXEN0);
	/* Parity and Stop bit*/
	*uart_reg.UCSRC_Reg |= (uart_cfg->parity)<<UPM00;
	*uart_reg.UCSRC_Reg |= (uart_cfg->stopBits)<<USBS0;

	if (uart_cfg->numberOfDataBits == 9)
	{
		*uart_reg.UCSRB_Reg |= 0x04;
		*uart_reg.UCSRC_Reg |=  0x06;
	}
	...
	{
		*uart_reg.UCSRC_Reg |= 0x02;
	}
	else if(uart_cfg->numberOfDataBits == 7)
	{
		*uart_reg.UCSRC_Reg |=  0x04;
	}
	else
	{
		*uart_reg.UCSRC_Reg |= 0x06;
...
BOOLEAN EF_BOOLEAN_UART_GetChar(U8_t * returnedValue)
{
	...
	/* make UART unstuck*/
    while( (!(*uart_reg.UCSRA_Reg & (1 << RXC0))) && (!EF_BOOLEAN_TimerCheck(UART_TIMER_ID)) );
...
BOOLEAN EF_BOOLEAN_UART_PutChar(U8_t data)
{
	...
	/* make UART unstuck */
    while( (!(*uart_reg.UCSRA_Reg & (1 << UDRE0))) && (!EF_BOOLEAN_TimerCheck(UART_TIMER_ID)) );
    /* extracting from the previous loop because of receiving byte or Time out*/
    EF_void_TimerStop(UART_TIMER_ID);
    EF_void_TimerReset(UART_TIMER_ID);

	if(*uart_reg.UCSRA_Reg & (1 << UDRE0))
	{
...
U8_t EF_u8_UART_WaitChar()
{
    ...
    while((!(*uart_reg.UCSRA_Reg & (1 << RXC0))) );
...
{
	while(!(*uart_reg.UCSRA_Reg & (1 << UDRE0)));  /* wait until UDR ready */
	*uart_reg.UDR_Reg = data;    				  /* send character 	  */
}
